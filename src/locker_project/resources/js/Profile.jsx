...
export default function Profile() {
    ...
    const handleUploadCV = async (e) => {
        ...
        if (!file) return;
        if (file.size > 5 * 1024 * 1024) { alert('Ukuran CV maksimal 5MB'); return; }
        if (file.type !== 'application/pdf') { alert('CV harus berformat PDF'); return; }
    ...
    const handleDeleteCV = async () => {
        if (!confirm('Hapus CV Anda?')) return;
        try {
    ...
    const handleUploadCertFile = async (e) => {
        ...
        try {
            ...
            alert('Gagal mengunggah file sertifikat');
        } finally {
    ...
    if (isLoading) { return (<div className="min-h-screen bg-gray-50 flex justify-center items-center"> <span className="text-gray-400 font-medium animate-pulse">Memuat profil...</span> </div>); }
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans pb-12 relative overflow-x-hidden">
            ...
            {/* Modal Edit Intro */}
            {isEditModalOpen && (<div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col"><div className="flex justify-between items-center p-5 border-b border-gray-200"><h3 className="text-xl font-bold text-gray-900">Edit Profil</h3><button onClick={() => setIsEditModalOpen(false)} className="text-gray-400 hover:text-gray-600"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button></div><div className="overflow-y-auto p-5"><form id="edit-intro-form" onSubmit={handleSaveIntro} className="flex flex-col gap-4"><div><label className="block text-sm font-semibold text-gray-700 mb-1">Nama Lengkap *</label><input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div><div><label className="block text-sm font-semibold text-gray-700 mb-1">Headline *</label><input type="text" placeholder="Cth: Mahasiswa Informatika" value={formData.headline} onChange={(e) => setFormData({ ...formData, headline: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div><div><label className="block text-sm font-semibold text-gray-700 mb-1">Lokasi *</label><input type="text" placeholder="Cth: Surakarta" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div><div><label className="block text-sm font-semibold text-gray-700 mb-1">Posisi Saat Ini</label><input type="text" value={formData.current_position} onChange={(e) => setFormData({ ...formData, current_position: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div><div><label className="block text-sm font-semibold text-gray-700 mb-1">Instansi/Universitas</label><input type="text" value={formData.education} onChange={(e) => setFormData({ ...formData, education: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div></form></div><div className="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50"><button type="button" onClick={() => setIsEditModalOpen(false)} className="px-5 py-2 text-gray-600 font-semibold rounded-full hover:bg-gray-200">Batal</button><button type="submit" form="edit-intro-form" className="px-5 py-2 bg-[#8100D1] text-white font-semibold rounded-full hover:bg-purple-800">Simpan</button></div></div></div>)}
                ...
                <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center p-5 border-b border-gray-200">
                            <h3 className="text-xl font-bold text-gray-900">{expForm.id ? 'Edit Pengalaman' : 'Tambah Pengalaman'}</h3>
                            <button onClick={() => setIsExpModalOpen(false)} className="text-gray-400 hover:text-gray-600 focus:outline-none"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                        </div>
                        <div className="overflow-y-auto p-5">
                            <form id="exp-form" onSubmit={handleSaveExperience} className="flex flex-col gap-4">
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Jabatan *</label><input type="text" value={expForm.title} onChange={(e) => setExpForm({ ...expForm, title: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Nama Perusahaan *</label><input type="text" value={expForm.company_name} onChange={(e) => setExpForm({ ...expForm, company_name: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Lokasi</label><input type="text" value={expForm.location} onChange={(e) => setExpForm({ ...expForm, location: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-sm font-semibold text-gray-700 mb-1">Mulai (Bulan & Tahun)</label><input type="text" placeholder="Cth: Jan 2023" value={expForm.start_date} onChange={(e) => setExpForm({ ...expForm, start_date: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div>
                                    <div><label className="block text-sm font-semibold text-gray-700 mb-1">Berakhir (Kosongkan jika masih)</label><input type="text" placeholder="Cth: Des 2023" value={expForm.end_date} onChange={(e) => setExpForm({ ...expForm, end_date: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div>
                                </div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Deskripsi</label><textarea value={expForm.description} onChange={(e) => setExpForm({ ...expForm, description: e.target.value })} rows="4" className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none"></textarea></div>
                            </form>
                        </div>
                        <div className="p-5 border-t border-gray-100 flex justify-between gap-3 bg-gray-50">
                            {expForm.id ? (
                                <button type="button" onClick={() => handleDeleteExperience(expForm.id)} className="px-5 py-2 text-red-600 font-semibold rounded-full hover:bg-red-50">Hapus</button>
                            ) : <div></div>}
                            <div className="flex gap-3">
                                <button type="button" onClick={() => setIsExpModalOpen(false)} className="px-5 py-2 text-gray-600 font-semibold rounded-full hover:bg-gray-200">Batal</button>
                                <button type="submit" form="exp-form" className="px-5 py-2 bg-[#8100D1] text-white font-semibold rounded-full hover:bg-purple-800">Simpan</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {isEduModalOpen && (
                <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center p-5 border-b border-gray-200">
                            <h3 className="text-xl font-bold text-gray-900">{eduForm.id ? 'Edit Pendidikan' : 'Tambah Pendidikan'}</h3>
                            <button onClick={() => setIsEduModalOpen(false)} className="text-gray-400 hover:text-gray-600 focus:outline-none"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                        </div>
                        <div className="overflow-y-auto p-5">
                            <form id="edu-form" onSubmit={handleSaveEducation} className="flex flex-col gap-4">
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Institusi/Universitas *</label><input type="text" value={eduForm.school} onChange={(e) => setEduForm({ ...eduForm, school: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Gelar</label><input type="text" placeholder="Cth: Sarjana (S1)" value={eduForm.degree} onChange={(e) => setEduForm({ ...eduForm, degree: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Bidang Studi/Jurusan</label><input type="text" placeholder="Cth: Teknik Informatika" value={eduForm.field_of_study} onChange={(e) => setEduForm({ ...eduForm, field_of_study: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-sm font-semibold text-gray-700 mb-1">Tahun Mulai</label><input type="text" placeholder="Cth: 2020" value={eduForm.start_date} onChange={(e) => setEduForm({ ...eduForm, start_date: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" required /></div>
                                    <div><label className="block text-sm font-semibold text-gray-700 mb-1">Tahun Selesai</label><input type="text" placeholder="Cth: 2024" value={eduForm.end_date} onChange={(e) => setEduForm({ ...eduForm, end_date: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none" /></div>
                                </div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Deskripsi Kegiatan/Prestasi</label><textarea value={eduForm.description} onChange={(e) => setEduForm({ ...eduForm, description: e.target.value })} rows="4" className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#8100D1] outline-none"></textarea></div>
                            </form>
                        </div>
                        <div className="p-5 border-t border-gray-100 flex justify-between gap-3 bg-gray-50">
                            {eduForm.id ? (
                                <button type="button" onClick={() => handleDeleteEducation(eduForm.id)} className="px-5 py-2 text-red-600 font-semibold rounded-full hover:bg-red-50">Hapus</button>
                            ) : <div></div>}
                            <div className="flex gap-3">
                                <button type="button" onClick={() => setIsEduModalOpen(false)} className="px-5 py-2 text-gray-600 font-semibold rounded-full hover:bg-gray-200">Batal</button>
                                <button type="submit" form="edu-form" className="px-5 py-2 bg-[#8100D1] text-white font-semibold rounded-full hover:bg-purple-800">Simpan</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col relative">
                        ...
                        <div className="flex justify-between items-center p-5 border-b border-gray-200">
                            <h3 className="text-xl font-bold text-gray-900">
                                {activePlatform && !showActionPrompt ? `Hubungkan ${activePlatform.toUpperCase()}` : userData.name}
                            </h3>
                            ...
                            {activePlatform && !showActionPrompt ? (
                                ...
                                <form onSubmit={handleSaveIndividualContact} className="space-y-5 py-2">
                                    <div className="bg-purple-50 text-[#8100D1] p-3 rounded-lg text-xs font-medium">
                                        💡 Masukkan detail akun Anda. Sistem otomatis mengaitkannya langsung ke Aplikasi Handphone.
                                    </div>
                                    ...
                                    {activePlatform === 'whatsapp' && (
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="text" placeholder="Contoh: 628820035..." value={platformInputValue} onChange={(e) => setPlatformInputValue(e.target.value)} className="w-full border border-gray-300 rounded-lg p-3 pl-7 text-sm focus:ring-2 focus:ring-[#8100D1] outline-none" required />
                                            </div>
                                        </div>
                                    )}
                                    {activePlatform === 'instagram' && (
                                        <div>
                                            ...
                                            <input type="text" placeholder="Contoh: rizky_rafif (Tanpa @)" value={platformInputValue} onChange={(e) => setPlatformInputValue(e.target.value)} className="w-full border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#8100D1] outline-none" required />
                                        </div>
                                    )}
                                    {activePlatform === 'facebook' && (
                                        <div>
                                            <label className="block text-sm font-semibold text-gray-700 mb-1">Username / ID Profil Facebook</label>
                                            <input type="text" placeholder="Contoh: rafif.putra" value={platformInputValue} onChange={(e) => setPlatformInputValue(e.target.value)} className="w-full border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#8100D1] outline-none" required />
                                        </div>
                                    )}
                                    {activePlatform === 'github' && (
                                        <div>
                                            <label className="block text-sm font-semibold text-gray-700 mb-1">Username GitHub</label>
                                            <input type="text" placeholder="Contoh: rafifputra" value={platformInputValue} onChange={(e) => setPlatformInputValue(e.target.value)} className="w-full border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#8100D1] outline-none" required />
                                        </div>
                                    )}

                                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                                        <button type="button" onClick={() => setActivePlatform(null)} className="px-5 py-2 text-sm text-gray-600 font-semibold rounded-full hover:bg-gray-100">Batal</button>
                                        <button type="submit" className="px-5 py-2 text-sm bg-[#8100D1] text-white font-semibold rounded-full hover:bg-purple-800">Simpan Tautan</button>
                                    </div>
                                </form>
                            ) : activePlatform && showActionPrompt ? (
                                ...
                                <div className="text-center py-6 space-y-5">
                                    <p className="text-gray-700 text-sm font-medium">
                                        Tautan <span className="font-bold text-[#8100D1]">{activePlatform.toUpperCase()}</span> sudah dikonfigurasikan. Pilih tindakan Anda:
                                    </p>
                                    ...
                                    <button type="button" onClick={() => setActivePlatform(null)} className="text-xs text-gray-400 underline block mx-auto pt-4">Kembali</button>
                                </div>
                            ) : (
                                // --- MODE LIHAT KONTAK (DAFTAR TAUTAN) ---
                                <div className="space-y-8">
                                    <h4 className="text-lg font-bold text-gray-900 border-l-4 border-[#8100D1] pl-3">Tautan Langsung</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                                        ...
                                        <a href={getGmailUrl(userData.email)} className={`flex items-center gap-4 group cursor-pointer ${!userData.email ? 'opacity-50 pointer-events-none' : ''}`}>
                                            ...
                                            <div className="flex-1">
                                                <p className="text-sm font-semibold text-gray-900 group-hover:text-red-700 transition">Kirim Email</p>
                                                <p className="text-xs text-gray-500 mt-0.5">{userData.email || 'Belum diatur'}</p>
                                            </div>
                                        </a>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 mt-8 space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                    ...
                    <div className="px-6 sm:px-8 pb-8 relative">
                        ...
                        <div className="absolute top-4 right-6 flex gap-2">
                            <button onClick={handleShareProfile} title="Bagikan Profil" className="p-2 text-gray-500 hover:text-[#8100D1] hover:bg-purple-50 rounded-full transition-colors focus:outline-none">
                                <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
                            </button>
                            <button onClick={openEditModal} title="Edit Profil" className="p-2 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-colors focus:outline-none"><svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                        </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                        <div className="flex border-b border-gray-200">
                            <button onClick={() => setActiveTab('mutuals')} className={`flex-1 py-4 font-bold text-sm transition-colors ${activeTab === 'mutuals' ? 'text-[#8100D1] border-b-2 border-[#8100D1]' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                                Koneksi Saya ({connections.length + pendingRequests.length})
                            </button>
                            <button onClick={() => setActiveTab('explore')} className={`flex-1 py-4 font-bold text-sm transition-colors ${activeTab === 'explore' ? 'text-[#8100D1] border-b-2 border-[#8100D1]' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                                Jelajahi Profil Lain
                            </button>
                        </div>
                        ...
                        <div className="p-6 border-b border-gray-100 bg-gray-50 flex items-center gap-3">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#8100D1] text-sm"
                                />
                            </div>
                        </div>
                        ...
                        <div className="p-6">
                            {isConnLoading ? (
                                <div className="flex justify-center py-8"><span className="animate-pulse text-gray-400 font-medium">Memuat koneksi...</span></div>
                            ) : activeTab === 'mutuals' ? (
                                <div className="space-y-6">
                                    {pendingRequests.length > 0 && !searchConnQuery && (
                                        <div>
                                            <h4 className="text-sm font-bold text-gray-800 mb-4 border-l-4 border-[#8100D1] pl-2">Permintaan Masuk</h4>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {pendingRequests.map(req => (
                                                    <div key={req.connection_id} className="flex items-center gap-4 p-4 border border-gray-100 rounded-lg bg-gray-50">
                                                        ...
                                                        <div className="flex-1 overflow-hidden">
                                                            <h5 onClick={() => navigate('/profile/' + req.user_id)} className="font-bold text-gray-900 text-sm truncate cursor-pointer hover:text-[#8100D1] transition">{req.name}</h5>
                                                            <p className="text-xs text-gray-500 truncate">{req.headline || 'Job Seeker'}</p>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            ...
                                                            <button onClick={() => handleRespond(req.connection_id, 'reject')} className="p-2 bg-gray-200 text-gray-600 rounded-full hover:bg-gray-300 transition"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                    ...
                                    <div>
                                        {pendingRequests.length > 0 && !searchConnQuery && <h4 className="text-sm font-bold text-gray-800 mb-4 border-l-4 border-[#8100D1] pl-2 mt-6">Koneksi Aktif</h4>}
                                        {filteredConnections.length > 0 ? (
                                            <div className="flex overflow-x-auto gap-4 pb-4 snap-x [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                                {filteredConnections.map(conn => (
                                                    <div key={conn.connection_id} className="snap-start flex-shrink-0 w-48 sm:w-56 flex flex-col border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition bg-white relative">
                                                        <div
                                                        ...
                                                        <div className="px-4 pb-4 flex flex-col items-center flex-1 relative">
                                                            <div
                                                                onClick={() => navigate('/profile/' + conn.user_id)}
                                                                className="w-16 h-16 rounded-full overflow-hidden bg-white p-1 -mt-8 mb-2 shadow-sm cursor-pointer z-10"
                                                            >
                                                            ...
                                                            <h5
                                                                onClick={() => navigate('/profile/' + conn.user_id)}
                                                                className="font-bold text-gray-900 text-sm line-clamp-1 cursor-pointer hover:text-[#8100D1] transition text-center w-full"
                                                            >
                                                                {conn.name}
                                                            </h5>
                                                            <p className="text-xs text-gray-500 line-clamp-2 mt-1 min-h-[32px] text-center w-full">{conn.headline || 'Job Seeker'}</p>

                                                            <button className="mt-4 w-full py-1.5 bg-white border border-[#8100D1] text-[#8100D1] text-xs font-bold rounded-full hover:bg-purple-50 transition shadow-sm">
                                                                Kirim Pesan
                                                            </button>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="text-center py-8">
                                                <p className="text-gray-500 text-sm mb-3">Tidak ada koneksi yang ditemukan.</p>
                                                {!searchConnQuery && <button onClick={() => setActiveTab('explore')} className="px-5 py-2 bg-purple-50 text-[#8100D1] text-sm font-bold rounded-full hover:bg-purple-100 transition">Mulai Jelajahi</button>}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    {filteredSuggestions.length > 0 ? (
                                        <div className="flex overflow-x-auto gap-4 pb-4 snap-x [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                            {filteredSuggestions.map(user => (
                                                <div key={user.user_id} className="snap-start flex-shrink-0 w-48 sm:w-56 flex flex-col border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition bg-white relative">
                                                    <div
                                                    ...
                                                    <div className="px-4 pb-4 flex flex-col items-center flex-1 relative">
                                                        ...
                                                        <p className="text-xs text-gray-500 line-clamp-2 mt-1 min-h-[32px] text-center w-full">{user.headline || 'Job Seeker'}</p>

                                                        <button onClick={() => handleConnect(user.user_id)} className="mt-4 w-full py-1.5 bg-white border border-[#8100D1] text-[#8100D1] text-xs font-bold rounded-full hover:bg-purple-50 transition shadow-sm">
                                                            + Terhubung
                                                        </button>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="text-center py-8 text-gray-500 text-sm">Tidak ada saran profil yang sesuai.</div>
                                    )}
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                    <div className="flex justify-between items-center p-6 border-b border-gray-100">
                        ...
                        <button onClick={openAddExp} className="p-2 text-gray-500 hover:text-[#8100D1] hover:bg-purple-50 rounded-full transition"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg></button>
                    </div>
                    <div className="p-6">
                                    ...
                                    <div key={exp.id || index} className={`flex gap-4 ${index !== arr.length - 1 ? 'border-b border-gray-100 pb-6' : ''}`}>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <button onClick={() => openEditExp(exp)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full transition"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                                            </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                    <div className="flex justify-between items-center p-6 border-b border-gray-100">
                        ...
                        <button onClick={openAddEdu} className="p-2 text-gray-500 hover:text-[#8100D1] hover:bg-purple-50 rounded-full transition"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg></button>
                    </div>
                    <div className="p-6">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-800">{edu.degree}{edu.field_of_study ? `, ${edu.field_of_study}` : ''}</p>
                                                    <p className="text-xs text-gray-500 mt-1">{edu.start_date} - {edu.end_date || 'Saat ini'}</p>
                                                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-full">
                        <div className="flex justify-between items-center p-6 border-b border-gray-100">
                            ...
                            <button onClick={openAddCert} className="p-2 text-gray-500 hover:text-[#8100D1] hover:bg-purple-50 rounded-full transition"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg></button>
                        </div>
                        <div className="p-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-full">
                        <div className="flex justify-between items-center p-6 border-b border-gray-100">
                            ...
                            {!cvUrl && (
                                <button onClick={() => cvInputRef.current?.click()} className="p-2 text-gray-500 hover:text-[#8100D1] hover:bg-purple-50 rounded-full transition"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg></button>
                            )}
                        </div>
                        <div className="p-6">
                            {cvUrl ? (
                                <div className="border border-gray-200 rounded-xl p-4 flex items-center justify-between bg-gray-50">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-gray-900 text-sm">Resume/CV Anda</p>
                                            <a href={cvUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-[#8100D1] font-semibold hover:underline">Lihat Dokumen</a>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => cvInputRef.current?.click()} className="text-sm font-semibold text-gray-600 hover:text-gray-900 border border-gray-300 rounded-lg px-3 py-1.5 bg-white shadow-sm hover:bg-gray-50 transition">Perbarui</button>
                                        <button onClick={handleDeleteCV} className="text-sm font-semibold text-red-600 hover:text-red-700 border border-red-200 rounded-lg px-3 py-1.5 bg-red-50 hover:bg-red-100 transition">Hapus</button>
                                    </div>
                                </div>
                            ) : (
                                <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 flex flex-col items-center justify-center text-center">
                                    ...
                                    <h3 className="font-bold text-gray-900 mb-1">Unggah CV Anda</h3>
                                    <p className="text-sm text-gray-500 mb-4">PDF, maks 5MB</p>
                                    <button onClick={() => cvInputRef.current?.click()} disabled={isUploadingCv} className="px-5 py-2 bg-white border border-[#8100D1] text-[#8100D1] font-bold text-sm rounded-full hover:bg-purple-50 transition shadow-sm">
                                        {isUploadingCv ? 'Mengunggah...' : 'Pilih File'}
                                    </button>
                                </div>
                            )}
                            <input type="file" ref={cvInputRef} onChange={handleUploadCV} accept=".pdf" className="hidden" />
                        </div>
                    </div>
                </div>
                    ...
                    {isCertModalOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                            <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
                                <div className="flex justify-between items-center p-6 border-b border-gray-100">
                                    <h3 className="font-bold text-gray-800 text-lg">{certForm.id ? 'Edit Sertifikasi' : 'Tambah Sertifikasi'}</h3>
                                ...
                                <div className="p-6 overflow-y-auto custom-scrollbar">
                                    <form id="certForm" onSubmit={handleSaveCertification} className="space-y-4">
                                        <div>
                                            <label className="block text-sm font-semibold text-gray-700 mb-1.5">Nama Sertifikasi *</label>
