...
export default function ProfilePerusahaan() {
    ...
    const handleSaveIntro = async (e) => { 
        ...
        try { 
            ...
            window.alert('Gagal menyimpan profil perusahaan.'); 
        } 
    };
            ...
            {isEditModalOpen && (
                <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/40 dark:bg-black/60 backdrop-blur-md p-4">
                    <div className="bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.1)] border border-white/40 dark:border-white/5 w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center p-5 border-b border-gray-200/50 dark:border-slate-800">
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white">Edit Profil Perusahaan</h3>
                            <button onClick={() => setIsEditModalOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 bg-white/50 dark:bg-slate-800 hover:bg-white dark:hover:bg-slate-700 rounded-full p-2 transition shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                        </div>
                        <div className="overflow-y-auto p-6">
                            <form id="edit-intro-form" onSubmit={handleSaveIntro} className="flex flex-col gap-5">
                                <div><label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nama Perusahaan *</label><input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-[#8100D1]/30 focus:border-[#8100D1] outline-none shadow-sm transition" required /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Bidang Industri *</label><input type="text" placeholder="Cth: Teknologi Informasi" value={formData.headline} onChange={(e) => setFormData({ ...formData, headline: e.target.value })} className="w-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-[#8100D1]/30 focus:border-[#8100D1] outline-none shadow-sm transition" required /></div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div><label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Lokasi Pusat</label><input type="text" placeholder="Cth: Jakarta Selatan" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} className="w-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-[#8100D1]/30 focus:border-[#8100D1] outline-none shadow-sm transition" /></div>
                                    <div><label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Jumlah Karyawan</label><select value={formData.employee_count} onChange={(e) => setFormData({ ...formData, employee_count: e.target.value })} className="w-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-[#8100D1]/30 focus:border-[#8100D1] outline-none shadow-sm transition">
                                ...
                                <div><label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Tautan Website (opsional)</label><input type="url" placeholder="Cth: https://www.perusahaan.com" value={formData.website_url} onChange={(e) => setFormData({ ...formData, website_url: e.target.value })} className="w-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-[#8100D1]/30 focus:border-[#8100D1] outline-none shadow-sm transition" /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Tentang Perusahaan</label><textarea value={formData.current_position} onChange={(e) => setFormData({ ...formData, current_position: e.target.value })} rows="4" className="w-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-[#8100D1]/30 focus:border-[#8100D1] outline-none shadow-sm transition resize-none"></textarea></div>
                            </form>
                        </div>
                        <div className="p-6 border-t border-gray-200/50 dark:border-slate-800 flex justify-end gap-3 bg-gray-50/50 dark:bg-slate-800/30">
                            <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-6 py-2.5 text-gray-600 dark:text-gray-300 font-bold rounded-xl bg-white/80 dark:bg-slate-800/80 hover:bg-gray-100 dark:hover:bg-slate-700 shadow-sm transition-all">Batal</button>
                            <button type="submit" form="edit-intro-form" className="px-6 py-2.5 bg-gradient-to-r from-[#8100D1] to-purple-600 text-white font-bold rounded-xl hover:to-purple-700 shadow-[0_4px_15px_rgba(129,0,209,0.2)] hover:shadow-[0_4px_20px_rgba(129,0,209,0.4)] transition-all transform hover:-translate-y-0.5">Simpan Perubahan</button>
                        </div>
                    </div>
                </div>
            )}
                        ...
                        <div className="mt-2">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="font-semibold text-gray-700 dark:text-gray-200">{userData.follower_count.toLocaleString()}</span> Pengikut
                                </div>
                            </div>
                        </div>
                                ...
                                {[
                                    ...
                                    { id: 'lowongan', label: 'Lowongan Kerja' }
                                        ...
                                        <button 
                                            ...
                                            className={`py-3 text-sm font-bold whitespace-nowrap transition-all relative outline-none ${activeTab === tab.id ? 'text-[#8100D1] dark:text-[#c682ff]' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}
                                        >
                                        {tab.label}
                                        {activeTab === tab.id && (
                ...
                <div className="space-y-6">
                    ...
                    {activeTab === 'tentang' && (
                        <div className="bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_0_20px_rgba(255,255,255,0.02)] border border-gray-200/50 dark:border-white/5 p-6 sm:p-8 animate-fade-in-up relative group" style={{ animationDelay: '0.1s' }}>
                            <div className="absolute inset-0 pointer-events-none rounded-3xl border border-transparent group-hover:border-purple-500/10 transition-colors duration-500 z-50"></div>
                            <h2 className="text-xl font-extrabold text-gray-900 dark:text-white mb-5">Gambaran Umum</h2>
                            {userData.description ? (
                                ...
                                <div className="py-8 text-center bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-gray-300 dark:border-slate-700">
                                    <p className="text-gray-500 dark:text-gray-400 font-medium">Belum ada deskripsi profil perusahaan.</p>
                                    <button onClick={openEditModal} className="mt-3 text-[#8100D1] dark:text-[#c682ff] font-semibold text-sm hover:underline">Tambahkan Deskripsi</button>
                                </div>
                            )}
                            ...
                            {(userData.website_url || userData.employee_count || userData.npwp) && (
                                <div className="mt-8 pt-6 border-t border-gray-200/50 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                                    {userData.website_url && (
                                        <div>
                                            <p className="text-sm font-bold text-gray-900 dark:text-white mb-1">Situs Web</p>
                                            <a href={userData.website_url} target="_blank" rel="noopener noreferrer" className="text-[#8100D1] dark:text-[#c682ff] hover:underline text-sm break-all font-medium">{userData.website_url}</a>
                                        </div>
                                    )}
                                    {userData.employee_count && (
                                        <div>
                                            <p className="text-sm font-bold text-gray-900 dark:text-white mb-1">Ukuran Perusahaan</p>
                                            <p className="text-sm text-gray-600 dark:text-gray-400">{userData.employee_count}</p>
                                        </div>
                                    )}
                                    {userData.npwp && (
                                        <div>
                                            <p className="text-sm font-bold text-gray-900 dark:text-white mb-1">Nomor Induk Berusaha / NPWP</p>
                                            <p className="text-sm text-gray-600 dark:text-gray-400">{userData.npwp}</p>
                                        </div>
                                    )}
                                </div>
                            )}
                    ...
                    {activeTab === 'lowongan' && (
                        <div className="bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_0_20px_rgba(255,255,255,0.02)] border border-gray-200/50 dark:border-white/5 p-6 sm:p-8 animate-fade-in-up relative group" style={{ animationDelay: '0.2s' }}>
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-extrabold text-gray-900 dark:text-white">Lowongan yang Dibuka</h2>
                                <span className="bg-purple-100 dark:bg-purple-900/40 text-[#8100D1] dark:text-[#c682ff] text-xs font-bold px-3 py-1 rounded-full">{recentJobs.length} Lowongan</span>
                            </div>
                            ...
                            {isJobsLoading ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {recentJobs.map(job => (
                                        <div key={job.id} onClick={() => navigate(`/loker/${job.id}`)} className="border border-gray-200/50 dark:border-slate-800 bg-white/50 dark:bg-slate-800/30 rounded-2xl p-5 cursor-pointer hover:shadow-lg hover:shadow-purple-500/5 hover:border-[#8100D1]/30 transition-all group backdrop-blur-sm">
                                            <div className="flex gap-4">
                                                <div className="w-14 h-14 rounded-xl bg-white dark:bg-slate-900 border border-gray-200/50 dark:border-slate-700 overflow-hidden shrink-0 flex items-center justify-center p-1 shadow-sm">
                                                    {userData.avatar_url ? <img src={userData.avatar_url} alt="Logo" className="w-full h-full object-contain" /> : <div className="w-full h-full bg-gray-100 dark:bg-slate-800 rounded-lg"></div>}
                                                </div>
                                                <div className="flex-1">
                                                    <h3 className="font-bold text-gray-900 dark:text-white group-hover:text-[#8100D1] dark:group-hover:text-[#c682ff] transition line-clamp-1">{job.role}</h3>
                                                    <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-1.5">
                                                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                                        {job.location}
                                                    </p>
                                                    <div className="mt-3 flex gap-2">
                                                        <span className="bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300 text-xs font-semibold px-2.5 py-1 rounded-md">{job.type}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="py-12 text-center bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-gray-300 dark:border-slate-700">
                                    ...
                                    <h3 className="text-gray-900 dark:text-white font-bold mb-1">Belum Ada Lowongan</h3>
                                    <p className="text-gray-500 dark:text-gray-400 text-sm">Perusahaan ini belum memiliki lowongan pekerjaan yang aktif saat ini.</p>
                                </div>
                            )}
                        </div>
                    )}
