...
export default function PublicCompanyProfile({ userData }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-[#0B0F19] font-sans pb-12 relative overflow-x-hidden transition-colors duration-500">
            ...
            {/* --- HEADER NAV --- */}
            <div className="sticky top-0 z-[60] bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-xl border-b border-gray-200/50 dark:border-white/5 flex items-center gap-4 px-4 sm:px-6 h-16 w-full transition-colors duration-500 shadow-[0_4px_30px_rgba(0,0,0,0.02)]">
                <button onClick={() => navigate(-1)} className="p-2 -ml-2 text-gray-600 dark:text-gray-400 hover:text-[#8100D1] dark:hover:text-[#a055db] transition-colors rounded-full hover:bg-purple-50 dark:hover:bg-slate-800">
            ...
            <main className="max-w-[900px] mx-auto w-full pt-6 px-4 pb-20 flex flex-col gap-6">
                ...
                <div className="bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_0_20px_rgba(255,255,255,0.02)] border border-gray-200/50 dark:border-white/5 overflow-hidden relative animate-fade-in-up group z-10">
                    ...
                    <div className="px-6 sm:px-8 pb-8 relative z-10">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-4">
                            ...
                            <div className="relative z-10 -mt-[60px] sm:-mt-[70px] mb-4 sm:mb-0">
                                <div className="w-[120px] h-[120px] sm:w-[140px] sm:h-[140px] rounded-2xl bg-white dark:bg-slate-900 p-1.5 shadow-md border border-gray-100 dark:border-slate-700">
                                    <div className="w-full h-full bg-gray-50 dark:bg-slate-800 flex justify-center items-center overflow-hidden rounded-xl border border-gray-100 dark:border-slate-700">
                                        {userData.avatar_url ? (
                                            <img src={userData.avatar_url} alt="Profile" className="w-full h-full object-contain bg-white" />
                                        ) : (
                                            <span className="text-5xl text-[#8100D1] dark:text-[#a055db] font-black">{userData.name?.charAt(0)}</span>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mt-2">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <svg className="w-4 h-4 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
                                    <span className="font-semibold text-gray-700 dark:text-gray-300">{followerCount?.toLocaleString() || 0}</span> {t('company.followers')}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex gap-6 overflow-x-auto no-scrollbar">
                                {[
                                    ...
                                    >
                                        {tab.label}
                                        {activeTab === tab.id && (
                                            <div className="absolute bottom-0 left-0 w-full h-[3px] bg-[#8100D1] dark:bg-[#a055db] rounded-t-md"></div>
                                        )}
                                    </button>
                                ))}
                            </div>
                ...
                <div className="space-y-6 relative z-10">
                    {/* TAB TENTANG */}
                    {activeTab === 'tentang' && (
                        <div className="bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_0_20px_rgba(255,255,255,0.02)] border border-gray-200/50 dark:border-white/5 p-6 sm:p-8 animate-fade-in-up relative group">
                            <div className="absolute inset-0 pointer-events-none rounded-3xl border border-transparent group-hover:border-purple-500/10 transition-colors duration-500 z-50"></div>
                            <h2 className="text-xl font-extrabold text-gray-900 dark:text-white mb-5">{t('company.overview')}</h2>
                                ...
                                <div className="mt-8 pt-6 border-t border-gray-200/50 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                                    {userData.website_url && (
                                        <div>
                                            ...
                                            <a href={userData.website_url} target="_blank" rel="noopener noreferrer" className="text-[#8100D1] dark:text-[#a055db] hover:underline text-sm break-all font-medium">{userData.website_url}</a>
                                        </div>
                                    )}
                                    {userData.employee_count && (
                                        <div>
                                            <p className="text-sm font-bold text-gray-900 dark:text-white mb-1">{t('company.company_size')}</p>
                                            <p className="text-sm text-gray-600 dark:text-gray-400">{userData.employee_count}</p>
                                        </div>
                                    )}
                                    {userData.npwp && (
                                        <div>
                                            <p className="text-sm font-bold text-gray-900 dark:text-white mb-1">{t('company.npwp')}</p>
                        ...
                        <div className="bg-white/80 dark:bg-[#0B0F19]/80 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_0_20px_rgba(255,255,255,0.02)] border border-gray-200/50 dark:border-white/5 p-6 sm:p-8 animate-fade-in-up relative group">
                            ...
                            {isJobsLoading ? (
                                        ...
                                        >
                                            <div className="flex gap-4">
                                                ...
                                                <div className="flex-1">
                                                    <h3 className="font-bold text-gray-900 dark:text-white group-hover:text-[#8100D1] dark:group-hover:text-[#a055db] transition line-clamp-1">{job.role}</h3>
                                ...
                                <div className="py-12 text-center bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-gray-300 dark:border-slate-700 relative z-10">
                                    ...
                                    <p className="text-gray-500 dark:text-gray-400 text-sm">{t('company.no_jobs_desc')}</p>
                                </div>
                            )}
                        </div>
                    )}
